...
`FiniteMagmaStr` inherits `carrier -> set` from `MagmaStr` and `carrier -> finite set` from `FiniteSetStr`.
Since `finite set` is a subtype of `set`, a coherence proof is required:

```mizar
definition
  struct MagmaStr where
    field carrier -> set;
    field binop -> BinOp of carrier;
  end;

  struct FiniteStr where
    field carrier -> finite set;
  end;

  struct FiniteMagmaStr where
    field carrier -> finite set;
    field binop -> BinOp of carrier;
  end;

  inherit FiniteMagmaStr from MagmaStr where
    field carrier from carrier;
    coherence
    proof
      :: finite set is a subtype of set (upcast is valid)
      thus for M being FiniteMagmaStr holds M.carrier is set;
    end;
    field binop from binop;
  end;

  inherit FiniteMagmaStr from FiniteStr where
    field carrier from carrier;
  end;
end;
```
