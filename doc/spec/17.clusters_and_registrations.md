...
```ebnf
existential_registration ::= "cluster" adjective_list type_expression ";"
...
```mizar
registration
  :: Register that there exists a strict non empty Group
  cluster strict non empty Group;
  existence
  proof
    set G = SymmetricGroup(3);
    take G;
    thus G is strict non empty Group by GroupTheory.Sym3_is_Group;
  end;
end;
```
...
Existential registrations are required (or strongly recommended) in the following situations:
...
| Defining a mode with attributed base | `mode Prime is prime Integer;` | Warning at definition; error at use |
| Using an attributed type in a functor return | `func F(x) -> non zero Real` | Required for well-formedness |

> [!NOTE]
> Existence checks for modes follow a two-stage policy: a **warning** at mode declaration if no existential registration is found, and a **hard error** when the mode is subsequently used without one. See [Chapter 5 (Modes), §5.8](./05.modes.md#58-correctness-conditions) for details.
...
```ebnf
conditional_registration ::= "cluster" antecedent_adjectives "->" consequent_adjectives
                              "for" type_expression ";"
...
After this registration, the verifier automatically infers `finite` whenever it knows a set is `empty`:
...
registration
  ...
  cluster finite non empty -> complete for Lattice;
  coherence
  proof
    let L be Lattice;
...
let S be empty set;
S is countable;   :: automatically true (by chaining)
```
...
registration
  let G be Group;
  cluster cyclic -> abelian for Subgroup of G;
  coherence
  proof
...
```mizar
registration
  :: A non zero Real is non empty (as a set)
  cluster non zero -> non empty for Real;
  coherence proof ... end;
end;
```
...
```ebnf
functorial_registration ::= "cluster" functor_term "->" consequent_adjectives
                             "for" type_expression ";"
                             "coherence" justification ";" ;
...
registration
  let n be Nat;
  cluster n ! -> positive non zero for Nat;
  coherence
  proof
    let n be Nat;
...
### 17.5.3 Functorial Registration for User-Defined Functors
...
registration
  let G be Group;
  cluster center(G) -> abelian for Subgroup of G;
  coherence
  proof
    let G be Group;
...
registration
  ...
  cluster bool A -> non empty for set;
  coherence proof ... end;
end;
...
let X be non empty set;
set P = bool [: X, X :];
:: [: X, X :] is non empty (by §17.5.1)
:: bool [: X, X :] is non empty (by this registration, since [: X, X :] is non empty set)
```
...
```ebnf
reduction_registration ::= "reduce" term_expression "to" term_expression ";"
...
After this registration, the verifier automatically simplifies `n + 0` to `n` during type checking:

```mizar
let m be Nat;
set k = m + 0;
:: The verifier recognizes k = m (by reduction)
```
...
### 17.6.3 Reduction for Structure Operations
...
registration
  ...
  reduce (x qua Element of S.carrier) to x;
  reducibility
  proof
    let S be 1-sorted;
    let x be Element of S;
    thus (x qua Element of S.carrier) = x;
  end;
end;
```
...
| **Confluence** | The verifier does not guarantee confluence — users must ensure reductions do not introduce cycles |
...
> [!CAUTION]
> Circular reductions (e.g., `reduce A to B` and `reduce B to A`) cause non-termination in the verifier. The verifier should detect and reject such cycles during registration, but users are advised to avoid them.

## 17.7 Cluster Resolution Rules

The verifier applies cluster registrations during type inference to automatically derive attribute information. This section describes the resolution algorithm.

### 17.7.1 Resolution Algorithm

When the verifier encounters a type expression with attributes, it performs the following steps:

```
function resolve_clusters(type T with attributes A₁, ..., Aₙ):
  1. Initialize known_attrs = { A₁, ..., Aₙ }
  2. Repeat until fixpoint:
     a. For each conditional cluster (P₁ ... Pₖ -> Q₁ ... Qₘ for T'):
        - If T is a subtype of T'
        - And { P₁, ..., Pₖ } ⊆ known_attrs
        - Then known_attrs = known_attrs ∪ { Q₁, ..., Qₘ }
     b. For each functorial cluster (F(args) -> Q₁ ... Qₘ for T'):
        - If the term being typed matches F(args)
        - And T is a subtype of T'
        - Then known_attrs = known_attrs ∪ { Q₁, ..., Qₘ }
  3. Return known_attrs
```

### 17.7.2 Resolution Order

| Priority | Rule | Description |
|---|---|---|
| 1 | **Direct attributes** | Attributes explicitly stated in the type expression |
| 2 | **Inherited attributes** | Attributes inherited from structure parents |
| 3 | **Conditional clusters** | Attributes derived via conditional cluster chains |
| 4 | **Functorial clusters** | Attributes derived from functor result registrations |
| 5 | **Reductions** | Term simplifications applied during matching |

### 17.7.3 Fixpoint Computation

The resolution algorithm iterates until no new attributes can be derived. Termination is guaranteed because:
- The set of all registered attributes is finite
- Each iteration adds at least one new attribute or the process terminates
- No iteration removes attributes (monotonicity)

**Example of chained resolution:**
...
registration
  ...
  cluster countable -> Dedekind-infinite-or-finite for set;
  coherence proof ... end;
end;

:: Resolution for "empty set":
:: Step 1: known_attrs = { empty }
:: Step 2a: empty -> finite matches => known_attrs = { empty, finite }
:: Step 2a: finite -> countable matches => known_attrs = { empty, finite, countable }
:: Step 2a: countable -> Dedekind-infinite-or-finite matches
::          => known_attrs = { empty, finite, countable, Dedekind-infinite-or-finite }
:: Step 2a: no more matches => fixpoint reached
```

### 17.7.4 Subtype Compatibility in Resolution
...
:: The verifier derives: S is finite (cluster applies to subtypes of set)
```

### 17.7.5 Conflict Resolution

If two conditional clusters produce contradictory attributes (e.g., one derives `finite` and another derives `non finite`), the behavior is as follows:

1. **Both are recorded** — the verifier does not reject conflicting attributes at registration time
2. **Contradiction detection** — if both `A` and `non A` appear in the resolved attribute set, the verifier marks the type as **inconsistent**
3. **Usage error** — attempting to declare a variable of an inconsistent type results in a verifier error

> [!NOTE]
> Inconsistent clusters typically indicate a logical error in the registered axioms. The verifier should report a diagnostic identifying which cluster registrations led to the contradiction.
...
Cluster registrations generate proof obligations as described in [Chapter 16 (Theorems and Proofs), §16.5.3](./16.theorems_and_proofs.md#1653-cluster-registration-obligations). The following table summarizes the obligations:
...
:: The cluster "empty -> finite for set" is now available here
```
...
| `cluster strict non empty Group;` | `∃x. (is_Group(x) ∧ is_strict(x) ∧ ¬is_empty(x))` |
...
```
cluster finite non empty -> complete bounded for Lattice;
...
registration
  ...
  cluster commutative -> abelian for Subgroup of (R qua AddLoopStr);
  coherence proof ... end;

  let I be Ideal of R;
  cluster R / I -> Ring-like for DoubleLoopStr;
  coherence proof ... end;
end;
```
...
> [!NOTE]
> For ATP integration details and property encoding, see [Chapter 23 (Source Code Annotation and ATP Integration)](./23.source_code_annotation.md).
...
(* === Conditional Cluster === *)
conditional_registration ::= "cluster" antecedent_adjectives "->"
                              consequent_adjectives "for" type_expression ";"
                              "coherence" justification ";" ;

(* === Functorial Cluster === *)
functorial_registration  ::= "cluster" functor_term "->"
                              consequent_adjectives "for" type_expression ";"
                              "coherence" justification ";" ;
