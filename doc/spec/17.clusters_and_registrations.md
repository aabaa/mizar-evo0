...
registration
  ...
  cluster positive -> non zero for Real;
  coherence proof ... end;
end;
```
...
> [!NOTE]
> Reduction registrations generate universal equality axioms (see §17.9.4) and are also exported to ATP backends as equational axioms (see §17.9.6). ATPs can leverage these equalities for term rewriting and equational reasoning.
...
:: ATP receives:
::   Axiom 1: ∀x. (is_set(x) ∧ is_empty(x) → is_finite(x))
::   Axiom 2: ∀x. (is_set(x) ∧ is_finite(x) → is_countable(x))
