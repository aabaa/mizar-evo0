...
**Example 2 — Applying a mixed schema (func and pred parameters):**
...
proof
  ...
  thus for n being Nat holds P(n)
    by IterationInvariant[0, F, P], A1;
end;
```
...
| `let T be type extends M; T as N` (N ancestor of M) | No new axiom; `is_T(x) → is_M(x) → is_N(x)` by inheritance chain |
...
type_param      ::= identifier "be" ( "type" [ "extends" bound_type ] | type_expression ) ;
...
coercion_arg    ::= identifier "as" radix_type ;    (* well-formed iff identifier's bound inherits from radix_type *)
