...
definition
  ...
  theorem IterationInvariant:
    (for n being Nat holds P(F@@n(x0)))
  provided
    Init: P(x0) and
    Step: for y being A st P(y) holds P(F(y))
  proof
    defpred Q(n) means P(F@@n(x0));
    thus for n being Nat holds Q(n) by NatInduction[Q], QBase, QStep
    where
      QBase: proof thus Q(0) by Init; end,
      QStep: proof
        let n be Nat;
        assume Q(n);
        thus Q(n+1) by Step;
...
proof
  ...
  Init: P(0)
  proof thus (succ)@@0(0) >= 0; end;
  Step: for x being Nat st P(x) holds P(F(x))
  proof
    let x be Nat;
    assume A1: (succ)@@x(0) >= 0;
    thus (succ)@@(succ(x))(0) >= 0 by A1;
  end;
  thus for n being Nat holds P(n)
    by IterationInvariant[F, P], Init, Step;
end;
```
