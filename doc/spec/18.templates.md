...
A template is a `definition` block whose leading items are `let` declarations establishing the template parameters. All items in the block share the scope of these parameter declarations. This extends the standard Mizar scoping rule at the top of a `definition` block scopes over all items in that block — to multi-item template definitions.
...
Schema theorems are ordinary implication theorems. The antecedent conditions are labeled inside the proof body with `assume Label: ...`. Callers prove these conditions as labeled facts and cite them via `by NatInduction[P], Label1, Label2`.
...
> [!NOTE]
> For a comprehensive treatment of the three function representations (set-theoretic functions, `deffunc` term abbreviations, and schema functor parameters), see [Chapter 10 (Functors)](./10.functors.md).
...
> [!NOTE]
> The `such that` constraint on `let` applies to **type parameters** used in definition items and is verified at use sites. Theorem-level assumptions are expressed as antecedents of `implies` in the theorem statement itself.
...
definition
  ...
  attr ExponentDef: n is a_valid_exponent_of[G] means
    for g being Element of G holds g @@ n = neutral[G];
  :: Automatic synonym: n is G-valid_exponent
end;
```

Here `n` is a `Nat` and `G` is a `Group` — they are independent types. The attribute expresses a relationship between n and G that cannot be reduced to either one alone.

**Usage:**

```mizar
let G be finite Group;
assume 12 is a_valid_exponent[G];
...
definition
  ...
  attr DenseDef: S is dense_in[M] means
...
`S` is a `Subset` of the carrier set of `M`, but the same carrier set can carry many different metric structures. Hence `M` is a genuine independent parameter: it specifies the metric, not just the underlying set.
...
The `of`/`over` syntax in ordinary mode definitions (Chapter 7) is shorthand for single-parameter mode templates:

```mizar
:: Shorthand (Chapter 7 syntax — preferred for single parameters)
definition
  mode Subset of X is X-subset_like set;
end;
...
Here `I be Ideal of R` is the genuine template parameter — the same ring elements `a, b` can be congruent relative to different ideals. `I` is not determined by `a` or `b` alone.
...
The partial order axioms are expressed as antecedent conjuncts of the implication, avoiding the need for `is`-predicate reasoning on the schema functor parameter `P`.
...
The `defpred` (or `deffunc`) name acts as the scheme argument. Labels of proved antecedent conditions in the current context are passed as additional label references. Since schema theorems are ordinary implication theorems, the ATP derives the conclusion from the implication plus the supplied labels.
...
| Theorem hypothesis | `P(0) & (...) implies ...` | Antecedent conditions in schema theorems |
