...
When the structure provided at a use site has multiple inheritance paths to the bound mode, the caller must select the intended path with `T as M`:

```mizar
PermProduct[Ring as AddMagma]              :: Ring's additive structure
PermProduct[commutative Ring as MulMagma]  :: Ring's multiplicative structure
```

`T as M` is well-formed iff the structure inherits from M via some `inherit ... extends M` chain. Writing `PermProduct[Ring]` when Ring reaches the bound mode via multiple paths is an error.
...
A theorem using `mul` (Group's notation) applied as `[Ring as AddGroup]` displays as `add` (AddGroup's notation) at the use site.
...
:: Application: sum permutation in any Ring
PermProduct[Ring as AddMagma]

:: Application: product permutation in any commutative Ring
PermProduct[commutative Ring as MulMagma]
```
