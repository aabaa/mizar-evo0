...
The `of`/`over` parameterization in modes (Chapter 7) and structures (Chapter 5) is **syntactic sugar** for single-parameter type templates. Both forms are accepted:

```mizar
:: Shorthand (Chapter 7 syntax — preferred for simple single parameters)
definition
...
definition
  ...
  let F be func(A) -> A;
  let P be pred(A);
  theorem Invariant[F,P]:
    P(x0) & (for x being A st P(x) holds P(F(x)))
    implies for x being A holds P(F(x))
  proof
    assume Base: P(x0);
...
When a type parameter must satisfy an additional condition beyond its mode, state it with `such that`. **No proof appears at the definition site** — the proof obligation is deferred to each **use site**, where the caller must supply evidence that the constraint holds for the specific instance used.

```mizar
definition
  let F be Field;
  let V be VectorSpace[F] such that V is finite_dimensional;
  :: Constraint: verified at each use site, not here
  mode Basis[V] is finite linearly_independent Subset of V.carrier;
  :: Basis[V] is only meaningful for finite-dimensional spaces
end;
```
