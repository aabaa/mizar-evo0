...
* `equals` requires no proof obligations; existence and uniqueness follow immediately from the term.
...
Note: Existence checks for modes are **mandatory**. The system raises a **hard error** at the point of mode declaration if no existential registration is found for the base type. This guarantees that all types in the system are non-empty, preserving the soundness of the logic.
...
| `equals` | none | The term directly supplies the value; existence and uniqueness are immediate |
