...
Introduces a reasoning block whose result serves as justification for the current proof goal. Equivalent to `thus now ... end`.
...
- **Grouped references**: Multiple theorems from the same module with a shared prefix (e.g., `mml.foo.{Th1, Th2}`)
- **Bulk references**: All public theorems from a module (e.g., `mml.foo.*`); see [Chapter 16 (Theorems and Proofs), §16.5](./16.theorems_and_proofs.md#165-proof-justification-by) for trade-offs
