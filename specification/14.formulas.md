...
| `G is non empty` | `is_non_empty(G)` |
| `x is not finite` | `¬is_finite(x)` |

> [!NOTE]
> `non` in adjective chains produces a single predicate (`is_non_empty`), while `not` in `x is not finite` produces formula-level negation (`¬is_finite(x)`).
