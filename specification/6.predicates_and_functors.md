...
definition
  let G be Group;
  func center(G) -> Subgroup[G] means
    for x be Element[G] holds x in it iff
    for y be Element[G] holds x, y are_commute;
end
```
...
definition
  let G be Group;
  let H, K be Subgroup[G];
  func H * K -> Subset[G] means
    for x be Element[G] holds x in it iff
    ex h be Element[H] ex k be Element[K] st x = h * k;
end
```
