...
- [2. Lexical Structure](#2-lexical-structure)
  ...
  - [2.10. Whitespace and Formatting](#210-whitespace-and-formatting)
    ...
    - [2.10.2. Example of Formatted Code](#2102-example-of-formatted-code)
  ...
  - [2.11. Lexical Preprocessing](#211-lexical-preprocessing)
    ...
    - [2.11.3. Example: Context Sensitivity in Action](#2113-example-context-sensitivity-in-action)

## 2.1. Characters and Encoding

Mizar uses a clearly defined character set to accurately represent mathematical concepts while maintaining high readability and searchability of the code. The system adopts Unicode UTF-8 encoding, allowing for diverse language representation in comments and specific annotations.
...
By limiting code to ASCII characters, Mizar ensures consistency and compatibility, while Unicode UTF-8 encoding maintains future extensibility.
...
1. **Symbol Precedence:** Symbols defined in additional lexicons take precedence over identifiers if they form a valid identifier.

2. **Article-Specific Scope:** Each article can use a different set of symbols, allowing for flexibility in mathematical notation.
...
end environ equals ex exactly existence export extends
for from func
given
hence hereby holds
...
Numerals in Mizar are sequences of digits starting with a non-zero digit:
...
Integer literals are represented by numerals and are inferred as `natural number` type.
...
- Symbol names must not conflict with Mizar's reserved words or special symbols.
...
Mizar uses a Python-inspired import system, allowing access to elements from other modules. Various import syntax options are available (see Chapter 3 for details).

### 2.8.3. Best Practices

1. Use clear, descriptive file names
2. Organize related concepts into separate modules
3. Use namespace structure to create a logical hierarchy
4. Document each module's purpose and contents in file header comments
...
### 2.10.2. Example of Formatted Code
...
3. **Import Processing and Tokenization**
   ...
   - Tokens (keywords, identifiers, numerals, operators, symbols) identified and classified

4. **Recognition and Processing**
   - Keywords and reserved words identified
   - Identifiers and symbols recognized based on context
   - Numerals and special symbols processed
...
### 2.11.3. Example: Context Sensitivity in Action
