...
- [15. Statements](#15-statements)
  ...
  - [15.5 Type Management](#155-type-management)
    ...
    - [15.5.2 Term Reduction (`reduce`)](#1552-term-reduction-reduce)
...
> [!IMPORTANT]
> The `and` keyword here is a **statement-level condition separator**, not the formula-level conjunction `&`. Each `and`-separated proposition may have its own label. See [Chapter 14 (Formulas), §14.3.1](./14.formulas.md#1431-conjunction-) for the distinction.
...
assume A1: n >= 0;
reconsider n as Nat by A1, INT_1:3;
```
...
### 15.5.2 Term Reduction (`reduce`)

Declares that a term expression can be simplified to a canonical form automatically. This is part of registration blocks.
...
**Semantics:**
- The verifier automatically rewrites the left-hand term to the right-hand term during type-checking
- A `reducibility` correctness condition must prove the equality

> [!NOTE]
> Detailed cluster and registration syntax is defined in **Chapter 21 (Clusters)**.
...
> [!NOTE]
> `case` and `suppose` are interchangeable; `suppose` allows labeling the case assumption directly.
