...
definition
  ...
  pred CommuteDef: a, b commute means
...
definition
  let a, b, c be Real;
  pred BetweenDef: a < b < c means a < b & b < c;
end;
...
pred PrimeDef: prime n means ...;       :: prefix phrase
pred CommuteDef: a, b commute means ...;:: multi-infix phrase
```
...
Ambiguous symbols can also be resolved using the `qua` operator when involving structural types:

```mizar
let R be DoubleLoopStr;
assume (R qua AddLoopStr) is associative;
assume (R qua MulLoopStr) is commutative;
```
...
| `a, b do not commute` | `¬commute(a, b)` |
