...
| **Attributes** | Unary predicates that refine existing types | Ch.7 |
...
| **Clusters** | Registration mechanisms for type inference | §5.6 |
...
object
└── set
    └── (user-defined modes and structures)
```
...
:: Narrowing (explicit)
let x be Integer;
reconsider x as Nat;  :: Proof obligation: x >= 0
```
...
Detailed syntax and examples are provided in **Chapter 7 (Attributes)**.
