...
```ebnf
theorem_item  ::= theorem_decl | conjecture_decl ;
theorem_decl  ::= "theorem" label_identifier ":" formula justification ";" ;
conjecture_decl ::= "conjecture" label_identifier ":" formula ";" ;
```

**Theorem classes:**

| Class | Proof Required | Referenceable | Semantics |
|---|---|---|---|
| `theorem` | Yes | Yes (via label) | Fully verified proposition |
| `conjecture` | No | Yes (with warning) | Unverified assertion for top-down development |
...
- **Justification**: Either a `by` reference list (for trivially justified theorems) or a full `proof ... end` block. See [Chapter 15 (Statements)](./15.statements.md) for statement syntax within proofs.
...
A theorem whose proof references a conjecture (directly or transitively) is automatically downgraded to `conjecture` status. This prevents "verified" theorems from silently depending on unproven assumptions.

```mizar
conjecture C1: for n being Nat holds P[n];

:: Th2 is automatically downgraded to conjecture status
:: because it depends on unverified C1
...
- **Conjecture Propagation**: A theorem whose proof references a conjecture (directly or transitively) is automatically downgraded to `conjecture` status. The compiler tracks the dependency chain.
- **Error Handling**:
    - If a `proof` block contains unjustified terminal obligations, the theorem is marked as invalid.
    - Referencing a `conjecture` in `theorem` emits a compiler error: "Depends on unverified conjecture."
