...
:: Chained access
let R be Ring;
let zero_elem = R.additive.zero;
```

## 6.8 Logic Encoding Details

This section describes how structures map to First-Order Logic.

### 6.8.1 Structures as Predicates

Each structure `S` is encoded as a unary predicate `is_S(x)`.

| Mizar Source | FOL Encoding |
| --- | --- |
| `x is UnitalMagma` | `is_UnitalMagma(x)` |

### 6.8.2 Fields as Functions

Each field is encoded as a function symbol.
...
Inheritance relationships are encoded as implications:

```
inherit Derived from Base
⇔  ∀x. is_Derived(x) ⇒ is_Base(x)
```

### 6.8.4 Constructor Axioms

Constructors generate equality axioms for field assignments:

```
constructor OneStr(G) where it.carrier = G.carrier; ...
⇔  carrier(OneStr(G)) = carrier(G)
```
