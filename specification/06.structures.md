...
inherit_def   ::= "inherit" struct_name "from" parent_type "where"
                  { inherit_member } "end" ;
inherit_member ::= field_inherit | property_inherit ;
field_inherit    ::= "field" identifier "from" identifier ";" ;
property_inherit ::= "property" identifier "from" identifier ";" ;
...
**Type Specialization with Proof**:

When a field type is narrowed, a `coherence` proof is required:

```mizar
definition
  inherit RealMagma from Magma where
    field carrier from carrier;  :: Narrowing: set -> Subset of REAL
  coherence
    for x being Element of carrier holds x is Real;
  end
end
```
...
Consistency of shared fields (e.g., `carrier`) must be proven via `compatibility`:

```mizar
compatibility
  for x being ZeroOneStr holds
...
(* Inheritance *)
inherit_def    ::= "inherit" struct_name "from" parent_type "where"
                   { inherit_member } [ coherence_clause ] "end" ;
inherit_member ::= field_inherit | property_inherit ;
field_inherit  ::= "field" identifier "from" identifier ";" ;
property_inherit ::= "property" identifier "from" identifier ";" ;
parent_type    ::= struct_name | "set" ;
...
coherence_clause ::= "coherence" formula ";" ;

(* Compatibility for Diamond Inheritance *)
compatibility_def ::= "compatibility" formula ";" ;
